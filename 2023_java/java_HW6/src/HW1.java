...
public class HW1 extends JFrame {
	...
	private static String 	op1, op2, op3, operator1 = null, operator2 = null;
	private static boolean	signed = false, computed = false;
	...
	public HW1() {
		...
		setVisible(true);
		setFocusable(true);
		requestFocus();  //키보드 입력을 받을 준비가 되엇다.
	...
	private static void Execute(String bName) {
		...
		case "0": case "1": case "2":case "3":case "4":case "5":case "6":case "7":case "8":case "9":
			...
			if (outF.getText().equals("0") || computed) {
		        outF.setText(bName);
		        computed = false;
		        signed = false;
		    } else if (outF.getText().indexOf(".") == -1 || outF.getText().substring(outF.getText().indexOf(".") + 1).length() < 3) {
		...
		case PLUS: case MINUS: case DIV: case MULTI: case "%":
			if(operator1 == null) {
				op1 = outF.getText();
				operator1 = bName;
				fr.setTitle(String.format("%s %s", op1, operator1));
			} else {
				if(operator2 == null) {
					op2 = outF.getText();
					operator2 = bName;
					fr.setTitle(String.format("%s %s %s", fr.getTitle(), op2, operator2));
				}
			}
			outF.setText("0");
			break;
		case EQ:
	        if(operator2 == null) {
	            op2 = outF.getText();
	            String result = compute(op1, operator1, op2);
	            if ("Infinity".equals(result) || "NaN".equals(result)) {
	                outF.setText("Divide By Zero");
	            } else {
	                outF.setText(result);
	            }
	        } else {
	            op3 = outF.getText();
	            if( (operator1.equals(PLUS) || operator1.equals(MINUS)) &&
	                (operator2.equals(MULTI) || operator2.equals(DIV) || operator2.equals("%")) ) {
	                String result = compute(op1, operator1, compute(op2, operator2, op3));
	                if ("Infinity".equals(result) || "NaN".equals(result)) {
	                    outF.setText("Divide By Zero");
	                } else {
	                    outF.setText(result);
	                ...
	                String result = compute(compute(op1, operator1, op2), operator2, op3);
	                if ("Infinity".equals(result) || "NaN".equals(result)) {
	                    outF.setText("Divide By Zero");
	                } else {
	                    outF.setText(result);
	        ...
	        fr.setTitle(String.format("%s %s = %s", fr.getTitle(), (operator2==null)? op2:op3, outF.getText() ));
	        operator1 = operator2 = null;
	        computed = true;
	        signed = (outF.getText().charAt(0) == '-') ? true :false;
	        break;
			...
			if(operator2 == null) {
				op2 = outF.getText();
				outF.setText(compute(op1, operator1, op2));
			} else {
				op3 = outF.getText();
				if( (operator1.equals(PLUS) || operator1.equals(MINUS)) &&
					(operator2.equals(MULTI) || operator2.equals(DIV) || operator2.equals("%")) ) {
					outF.setText(compute(op1, operator1, compute(op2, operator2, op3)));
				} else {
					outF.setText(compute(compute(op1, operator1, op2), operator2, op3));
				}
			}
			fr.setTitle(String.format("%s %s = %s", fr.getTitle(), 
						(operator2==null)? op2:op3, outF.getText() ));
			operator1 = operator2 = null;
			computed = true;
			signed = (outF.getText().charAt(0) == '-') ? true :false;
			break;
		...
		case "DEL":
			...
			if((!signed && outF.getText().length() > 1) ||(signed && outF.getText().length() > 2)) {
		...
		case DOT:
			...
			break;
		case SIGN :
			// if(outF.getText().equals("0")) break;  //이걸 없애야하나?
			if(outF.getText().equals("0")) {
				outF.setText(outF.getText().substring(1));
			}
			if(signed) {
				outF.setText(outF.getText().substring(1));
			} else {
