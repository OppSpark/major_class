...
public class HW1 extends JFrame {
	...
	public HW1() {
		...
		//
		// while(curBullet.th.getState() != State.TERMINATED || curCha.th.getState() !=
		// State.TERMINATED|| curCar.th.getState() != State.TERMINATED);
	}
	...
	private class Car extends JLabel implements Runnable {
		...
		public void Down() {
			if (y + speed + car_icon.getIconHeight() > c.getWidth())
				return;
			y += speed;
			setPosition();
		}
	...
	private class Bullet extends JLabel implements Runnable {
		...
		public void Right() {
			if (bulnum == 2) {
				x += speed;
				setPosition();
			} else if (x + speed + icon.getIconWidth() > c.getWidth() || fired) {
				return;
			} else {
				x += speed;
				setPosition();
	...
	public class Cha extends JLabel {
		...
		public void Right() {
			if (Chx + speed + icon.getIconWidth() > c.getWidth() || fired)
				return;
			Chx += speed;
			setPosition();
		}
